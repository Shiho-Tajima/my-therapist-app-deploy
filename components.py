...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        # メイン画面には挨拶のみ表示
        # ★問題３　文言を修正。st.markdown→st.successに修正
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        # ★問題４　参照元ドキュメントのページ番号が取得できた場合にのみ、ページ番号を表示
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        for document in llm_response["context"][1:]:
            ...
            if "page" in document.metadata:
                ...
                print("サブドキュメント_ページ番号なし") # ★追記（デバッグ用）
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                # ページ番号が取得できない場合のための分岐処理
                if "page_number" in sub_choice:
                    # ★問題４「サブドキュメントのファイルパス」と「ページ番号」を表示
                    st.info(f"{sub_choice['source']}（ページNo.{sub_choice['page_number']}）", icon=icon)
                else:
                    # 「サブドキュメントのファイルパス」を表示
                    st.info(f"{sub_choice['source']}", icon=icon)
